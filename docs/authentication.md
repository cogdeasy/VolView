...
Volume Viewer itself does not specify any given authentication mechanism to use. How you let Volume Viewer access authenticated resources depends on individual deployment needs.

## Volume Viewer behind Login Portal

The easiest approach to securing a Volume Viewer instance is to host Volume Viewer behind an authentication portal. This will secure access to Volume Viewer, and allow Volume Viewer to automatically use existing user credentials to request protected data. Often the credentials are stored in a cookie.

## OAuth Tokens via URL

When accessing OAuth-protected resources, Volume Viewer by default does not add the `Authorization` header to requests. It is possible to tell Volume Viewer to add `Authorization` headers to all requests, via the `token` and `tokenUrl` parameters.
...
You can pass in the `token` URL parameter like so: `https://example.com/VolView/?token=XXXX`. When Volume Viewer sees this, it will extract the token from the URL and use it for all subsequent data requests. Volume Viewer will also strip the token from the URL to prevent saving the token into the browser history.

#### Common scenario: a hosted client reading protected data on another origin

A data portal links a user to a shared/hosted Volume Viewer (e.g. the public
`volview.kitware.app`) and passes a token so Volume Viewer can pull that user's
...
work, the **data server** (not Volume Viewer) must set CORS, and two details trip
people up:
...
- `Access-Control-Allow-Origin` must be a **single** value. A header echoing
  ...
  either `*` or the one Volume Viewer origin.
...
By default, Volume Viewer will make a `GET` request to the token URL. If another type of request is needed, you can configure it via the `tokenUrlMethod` parameter. For example, to make a `POST` request: `https:/example.com/VolView/?tokenUrl=https://example.com/userToken&tokenUrlMethod=POST`.
...
The remote save target (`save=`) is accepted only when its origin matches the origin serving Volume Viewer. A cross-origin `save=` URL is refused, and the save UI stays disabled — so **a `token=` (or `tokenUrl=`) plus a cross-origin `save=` does not work**, even though the same token will happily authorize cross-origin data requests. To save, host Volume Viewer on the same origin as the endpoint that receives the session.
