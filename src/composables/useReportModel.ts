    ...
    return {
      ...
      keyImage: finding.keyImage
        ? {
            ...
            // The position named here is the captured view's own, which is not
            // a slice for a cine finding or a 3D view.
            caption: [
              `${finding.title || 'Finding'} — ${finding.keyImage.viewName}`,
              finding.frame != null
                ? `frame ${finding.frame + 1}`
                : finding.keyImage.slice != null &&
